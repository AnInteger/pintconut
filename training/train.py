...
用法:
    python training/train.py --data training/dataset
    python training/train.py --data training/dataset --model yolov8n-seg.pt --epochs 100 --batch 16
"""
...
def generate_data_yaml(dataset_dir):
    ...
    包含单个类别 "beadboard"，指向 dataset_dir 下的 train/valid 子目录。
...
def main():
    parser = argparse.ArgumentParser(description="YOLO 分割模型训练")
    parser.add_argument(
        "--data", required=True,
    ...
    parser.add_argument(
        "--name", default="beadboard-v1",
        help="实验名称 (默认: beadboard-v1)",
    )
